...
export async function GET() {
  try {
    ...
    const { data: clientes, error: cliError } = await supabase
      .from('clientes')
      .insert([
        { business_id: bizId, nombre: 'Yareli Lozano', telefono: '3221234567', puntos: 10, email: 'yareli.lozano@gmail.com' },
        { business_id: bizId, nombre: 'Juan Pérez', telefono: '3227654321', puntos: 4, email: 'juan.perez@hotmail.com' },
        { business_id: bizId, nombre: 'María Rodríguez', telefono: '3229876543', puntos: 8, email: 'maria.rod@gmail.com' }
      ])
      .select()
    ...
    // Fechas pasadas distribuidas uniformemente para simular ticket promedio y frecuencia
    const haceDias = (dias: number) => new Date(Date.now() - dias * 24 * 60 * 60 * 1000).toISOString()

    await supabase.from('orders').insert([
      // Pedidos de Yareli (para simular historial robusto)
      ...
      // Pedido rechazado (para simular alerta de fraude)
      ...
      // 7. Pedidos PENDIENTES esperando aprobación (para probar el flujo anti-fraude)
    ...
    // 8. Inserción de eventos iniciales en tracking_events
