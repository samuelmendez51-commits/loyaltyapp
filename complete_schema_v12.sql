...
-- Asegurar la existencia de columnas en base preexistente
ALTER TABLE superadmins ADD COLUMN IF NOT EXISTS nombre TEXT NOT NULL DEFAULT 'Super Admin';
...
CREATE TABLE IF NOT EXISTS businesses (
  ...
  -- Anti-churn config
  dias_inactividad_churn INT DEFAULT 21,
  mensaje_rescate_whatsapp TEXT DEFAULT '¡Hola {nombre}! Te extrañamos en {negocio}. ¡Tienes {puntos} sellos esperándote! 🎁 Ven esta semana y recibe un beneficio especial.',
...
-- =============================================
-- TABLA: branches (Sucursales opcionales)
-- =============================================
...
-- =============================================
-- TABLA: business_users (Admins y Cajeros/Empleados)
-- =============================================
...
CREATE TABLE IF NOT EXISTS historial_puntos (
  ...
  tipo_movimiento TEXT NOT NULL,  -- 'suma', 'resta', 'canje', 'ajuste_manual', 'bono'
...
-- =============================================
-- TABLA: loyalty_rewards (Premios por Sellos Clásicos)
-- =============================================
...
-- =============================================
-- TABLA: reward_milestones (Hitos de Ruleta Gamificada V12)
-- =============================================
...
-- =============================================
-- TABLA: roulette_spins (Log de Giros y Cupones V12)
-- =============================================
...
CREATE TABLE IF NOT EXISTS menu_products (
  ...
  orden INT DEFAULT 0,
  es_upsell BOOLEAN DEFAULT FALSE,  -- Switch de venta cruzada V12
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- =============================================
-- PARCHE CRÍTICO PARA BASES DE DATOS PREEXISTENTES
-- Garantiza la existencia de columnas multi-tenant antes de indexar o crear funciones
-- =============================================
...
-- 1. Tabla branches
ALTER TABLE branches ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;

-- 2. Tabla business_users
ALTER TABLE business_users ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;
ALTER TABLE business_users ADD COLUMN IF NOT EXISTS branch_id UUID REFERENCES branches(id);
...
ALTER TABLE clientes ADD COLUMN IF NOT EXISTS qr_token TEXT UNIQUE DEFAULT encode(gen_random_bytes(12), 'hex');

-- 4. Tabla historial_puntos
ALTER TABLE historial_puntos ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id);

-- 5. Tabla loyalty_rewards
ALTER TABLE loyalty_rewards ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;

-- 6. Tabla reward_milestones
ALTER TABLE reward_milestones ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;

-- 7. Tabla roulette_spins
ALTER TABLE roulette_spins ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;

-- 8. Tabla credit_transactions
ALTER TABLE credit_transactions ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id);

-- 9. Tabla menu_groups
ALTER TABLE menu_groups ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;

-- 10. Tabla menu_products
ALTER TABLE menu_products ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;
...
ALTER TABLE orders ADD COLUMN IF NOT EXISTS sello_otorgado BOOLEAN DEFAULT FALSE;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS sello_aprobado BOOLEAN DEFAULT FALSE;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS sello_rechazado BOOLEAN DEFAULT FALSE;
ALTER TABLE orders ADD COLUMN IF NOT EXISTS aprobado_por UUID REFERENCES business_users(id);
ALTER TABLE orders ADD COLUMN IF NOT EXISTS estado TEXT DEFAULT 'pendiente';
ALTER TABLE orders ADD COLUMN IF NOT EXISTS canal TEXT DEFAULT 'whatsapp';

-- 12. Tabla tracking_events
ALTER TABLE tracking_events ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id);
ALTER TABLE tracking_events ADD COLUMN IF NOT EXISTS cliente_id UUID REFERENCES clientes(id);
ALTER TABLE tracking_events ADD COLUMN IF NOT EXISTS order_id UUID REFERENCES orders(id);
ALTER TABLE tracking_events ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES business_users(id);
ALTER TABLE tracking_events ADD COLUMN IF NOT EXISTS codigo_cupon TEXT UNIQUE;
ALTER TABLE tracking_events ADD COLUMN IF NOT EXISTS cupon_canjeado BOOLEAN DEFAULT FALSE;

-- 13. Tabla audit_logs
ALTER TABLE audit_logs ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id);
ALTER TABLE audit_logs ADD COLUMN IF NOT EXISTS user_id UUID REFERENCES business_users(id);

-- 14. Tabla business_schedules
ALTER TABLE business_schedules ADD COLUMN IF NOT EXISTS business_id UUID REFERENCES businesses(id) ON DELETE CASCADE;
...
BEGIN
  ...
  WHERE cliente_id = p_cliente_id
    ...
    AND event_type = 'created_pending'
    AND created_at >= NOW() - (p_minutos * INTERVAL '1 minute');
...
INSERT INTO businesses (
  ...
  dias_inactividad_churn = EXCLUDED.dias_inactividad_churn;
